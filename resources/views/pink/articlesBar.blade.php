        ...
        @if(!$portfolios->isEmpty())
            @foreach($portfolios as $portfolio)
                <div class="hentry-post group">
                    ...
                    <div class="text">
                        <a href="{{route('articles.show',['alias'=>$portfolio->alias])}}" title="{{$portfolio->title}}"
                           class="title">Section
                            shortcodes &amp; sticky posts!</a>
                        <p>{{str_limit($portfolio->text,130)}} </p>
                        <a class="read-more"
                           href="{{route('portfolios.show',['alias'=>$portfolio->alias])}}">&rarr; {{trans('ru.read_mo')}}</a>
                    </div>
                </div>
            @endforeach
        @endif
            ...
            @foreach($comments as $comment)
                <div class="the-post group">
                    ...
                    <span class="author"><strong><a
                                    href="#">{{isset( $comment->user)?$comment->user->name : $comment->name}}</a></strong> in</span>
