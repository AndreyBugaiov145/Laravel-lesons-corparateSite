<div id="content-single" class="content group">
        ...
        @if ($article)
            ...
            <div class="meta group">
                ...
                <p class="comments"><span><a href="#comments"
                                             title="Comment on This is the title of the first article. Enjoy it.">{{count($article->coments) ? count($article->coments) : '0'}}{{Lang::choice('ru.comments',count($article->coments))}} </a></span>
                </p>
            </div>
            <!-- post content -->
    ...
    <div id="comments">
        <h3 id="comments-title">
            <span>{{count($article->coments)}}</span> {{Lang::choice('ru.comments',count($article->coments))}}
        </h3>
        @if(count($article->coments)>0)
            @set($com,$article->coments->groupBy('parant_id'))

            <ol class="commentlist group">
                @foreach($com as $k => $coments)
                    @if($k !== 0)
                        @break
                    @endif
                    @include(env('THEME').'.coment',['items'=>$coments])
                @endforeach
